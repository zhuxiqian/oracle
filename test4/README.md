...
## 操作步骤
### 1、授权
ALTER USER user_zxq QUOTA UNLIMITED ON USERS;
ALTER USER user_zxq QUOTA UNLIMITED ON USERS02;
ALTER USER user_zxq ACCOUNT UNLOCK;

GRANT "CONNECT" TO user_zxq WITH ADMIN OPTION;
GRANT "RESOURCE" TO user_zxq WITH ADMIN OPTION;
ALTER USER  user_zxq DEFAULT ROLE "CONNECT","RESOURCE";

GRANT CREATE VIEW TO user_zxq WITH ADMIN OPTION;
GRANT CREATE SESSION TO   user_zxq;
GRANT CREATE TABLE TO user_zxq;
GRANT CREATE TABLESPACE TO  user_zxq;
GRANT CREATE VIEW TO  user_zxq;
实验截图:
![](pic1.png)

### 2、增加一个表空间 ZXQ
Create Tablespace ZXQ
datafile
'/home/oracle/app/oracle/oradata/orcl/pdborcl/ZXQ_user1.dbf'
  SIZE 100M AUTOEXTEND ON NEXT 256M MAXSIZE UNLIMITED,
'/home/oracle/app/oracle/oradata/orcl/pdborcl/ZXQ_user2.dbf'
  SIZE 100M AUTOEXTEND ON NEXT 256M MAXSIZE UNLIMITED
...
### 3、创建用户STUDY_ZXQ
CREATE USER STUDY_ZXQ IDENTIFIED BY 123
...
### 4、赋予ml视图权限
-- QUOTAS
ALTER USER STUDY_ZXQ QUOTA UNLIMITED ON USERS;
ALTER USER STUDY_ZXQ QUOTA UNLIMITED ON USERS02;
ALTER USER STUDY_ZXQ ACCOUNT UNLOCK;

-- ROLES
GRANT "CONNECT" TO STUDY_ZXQ WITH ADMIN OPTION;
GRANT "RESOURCE" TO STUDY_ZXQ WITH ADMIN OPTION;
ALTER USER STUDY_ZXQ DEFAULT ROLE "CONNECT","RESOURCE";

-- SYSTEM PRIVILEGES
GRANT CREATE VIEW TO STUDY_ZXQ WITH ADMIN OPTION;
实验截图:
![](pic4.png)
...
### 13、批量插入订单数据，注意ORDERS.TRADE_RECEIVABLE（订单应收款）的自动计算,注意插入数据的速度**
...
### 14、最后动态增加一个PARTITION_BEFORE_2018分区：
ALTER TABLE ORDERS
...
### 20、--收集表的统计信息dbms_stats.gather_table_stats
